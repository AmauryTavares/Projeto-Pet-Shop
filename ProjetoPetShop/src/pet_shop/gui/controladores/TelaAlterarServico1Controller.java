...
public class TelaAlterarServico1Controller implements Initializable {
	...
	public void atualizarTabela(List<Animal> lista) throws NadaEncontradoException {
		...
		tbColumnEspecie.setCellValueFactory(new PropertyValueFactory<>("espécie"));
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			atualizarTabela(fachada.listarTodosAnimais());
			tbViewAnimais.getSelectionModel().select(TelaGenServicosController.servicoAlterar.getAnimal());
		} catch (NadaEncontradoException e) {
			funcoes.alerta(AlertType.ERROR, "Ocorreu um problema!", "", e.getMessage());
		}
